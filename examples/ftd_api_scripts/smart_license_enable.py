...
def enable_license(host, port, user, passwd, license_type):
    """
    ...
    post_licenses(host, port, access_token, license_type)
    enabled_license = get_license_by_type(host, port, access_token, license_type)

    if enabled_license:
        print("Smart License with type \"{}\" successfully enabled.".format(license_type))
    else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 6:
        print("Usage: python ftd_api_scripts/smart_license_enable.py host port user passwd license_type")
        exit(1)
    ...
    try:
        enable_license(host, port, user, passwd, license_type)
        exit(0)
