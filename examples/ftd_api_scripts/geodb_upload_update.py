...
from ftd_api_resources.geodb_manual_update import send_geo_db_update_file, fetch_all_geo_db_jobs, fetch_geo_db_job_by_id


def main():
    # read the commandline arguments
    parser = argparse.ArgumentParser(description='Upload a GeoDB update file then track the update job.')
    ...
    update_code = send_geo_db_update_file(host, port, auth_token, file)
    if update_code != 200:
    ...
    jobs = fetch_all_geo_db_jobs(host, port, auth_token)
    if jobs is None:
    ...
    try_count = 0
    while try_count < 30:
        current_job = fetch_geo_db_job_by_id(host, port, auth_token, current_job_uuid)
