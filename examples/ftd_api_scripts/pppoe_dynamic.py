...
def pppoe_dynamic(host, port, user, passwd, intf_hw_name='GigabitEthernet1/1', vpdn_grp_name='my_pppoe_group', pppoe_user='my_pppoe_username', pppoe_password='my_pppoe_password', ppp_auth_type='PAP', pppoe_metric=1, obtain_default_route=True):
    """
    ...
    pppoe = {
        ...
        'ipIsDynamic': True,  #if True, then the ipAddress and Netmask should be set to '' in the ipv4 object of an interface object
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 5:
        print("Usage: python ftd_api_scripts/pppoe_dynamic.py host port user passwd [intf_hw_name, vpdn_grp_name, pppoe_user, pppoe_password, ppp_auth_type, pppoe_metric, obtain_default_route]")
        exit(1)
