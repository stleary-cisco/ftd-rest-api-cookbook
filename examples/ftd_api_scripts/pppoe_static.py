...
def pppoe_static(host, port, user, passwd, ip_addr='2.3.4.54', intf_hw_name='GigabitEthernet1/1', vpdn_grp_name='my_pppoe_group', pppoe_user='my_pppoe_username', pppoe_password='my_pppoe_password', ppp_auth_type='PAP', pppoe_metric=1, obtain_default_route=True):
    """
    ...
    pppoe = {
        ...
        'ipIsDynamic': False,  #if True, then the ipAddress and Netmask should be set to '' in the ipv4 object of an interface object
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 6:
        print("Usage: python ftd_api_scripts/pppoe_static.py host port user passwd ip_addr [intf_hw_name, vpdn_grp_name, pppoe_user, pppoe_password, ppp_auth_type, pppoe_metric, obtain_default_route]")
        exit(1)
    ...
    passwd = sys.argv[4]
    ip_addr = sys.argv[5]
    # intf_hw_name = sys.argv[6]
    # vpdn_grp_name = sys.argv[7]
    # pppoe_user = sys.argv[8]
    # pppoe_password = sys.argv[9]
    # ppp_auth_type = sys.argv[10]
    # pppoe_metric = sys.argv[11]
    # obtain_default_route = sys.argv[12]
    # if pppoe_dynamic(host=host, port=port, user=user, passwd=passwd, ip_addr=ip_addr, intf_hw_name=intf_hw_name, vpdn_grp_name=vpdn_grp_name, pppoe_user=pppoe_user, pppoe_password=pppoe_password, ppp_auth_type=ppp_auth_type, pppoe_metric=pppoe_metric, obtain_default_route=obtain_default_route):
