...
from ftd_api_resources.vdb_manual_update import send_vdb_update_file, fetch_all_vdb_jobs, fetch_vdb_job_by_id


def main():
    # read the commandline arguments
    parser = argparse.ArgumentParser(description='Upload a VDB update file then track the update job.')
    ...
    update_code = send_vdb_update_file(host, port, auth_token, file)
    if update_code != 200:
    ...
    jobs = fetch_all_vdb_jobs(host, port, auth_token)
    if jobs is None:
    ...
    try_count = 0
    while try_count < 30:
        current_job = fetch_vdb_job_by_id(host, port, auth_token, current_job_uuid)
